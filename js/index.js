  ...
  try {
    ...
    grid.innerHTML = exams
      .map((exam) => {
        ...
        // تم ضبط اسم المعامل ليكون ?exam= ليتوافق مع exam.js
        const examUrl = `${currentOrigin}${currentPath}exam.html?exam=${encodeURIComponent(examSlug)}`;
        ...
        <div class="idx-card" style="background: var(--bg-card, #1e2230); border: 1px solid var(--border, #2e3448); border-radius: 12px; padding: 1.5rem; display: flex; flex-direction: column; justify-content: space-between;">
          ...
          <div class="idx-card-actions" style="display: flex; gap: 0.5rem; flex-wrap: wrap; margin-top: 1.5rem;">
            <a href="exam.html?exam=${encodeURIComponent(examSlug)}" class="idx-btn primary" style="flex: 1; min-width: 120px; text-align: center; text-decoration: none; background: var(--primary, #3b82f6); color: #fff; padding: 0.6rem 1rem; border-radius: 8px; font-weight: bold; display: inline-flex; align-items: center; justify-content: center; gap: 0.5rem;">
